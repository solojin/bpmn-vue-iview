    ...
    >
      <x-form ref="xForm" v-model="formData" :config="formConfig">
        <template #params="scope">
          <Badge :value="scope.row.params ? scope.row.params.length : 0" type="primary">
            <Button size="small" @click="configParam(scope.$index)">配置</Button>
          </Badge>
        </template>
      </x-form>
      <span slot="footer" class="dialog-footer">
        <Button type="primary" size="default" @click="closeDialog">确 定</Button>
      </span>
    </Modal>
